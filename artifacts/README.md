...
.venv/bin/python scripts/check_checkpoint_one.py
...
- Outbound recency is 99 at every initial observation. In nonterminal transitions
  ...
  also persists at 99 or resets to 0. These are observed associations only; no
  real-world meaning is inferred. Features retain the raw value and a separate
  `is_99` indicator.
...
counts, consecutive repeats and waits, and occurred indicators for objection,
nudge, and scheduling; remaining decisions is `4-turn`.
...
Training-state smoke results are recorded in [`task2_checks.json`](task2_checks.json),
reproduced by `scripts/check_checkpoint_one.py`. Unit-test counts below come from
`.venv/bin/python -m pytest tests -q`:

- 53 Task 2 tests passed; 85 total tests passed.
...
- The encoder fitted only the policy-fit role: 52,323 decision rows from
  ...
  episodes into 113 dense features (schema version 2) with no nonfinite values. Transforming
...
The audit and split artifacts regenerated byte-for-byte on 2026-09-17. New public
interfaces include `validate_episode(rows)`, `audit(train_path, valid_path)`,
`write_splits(ids, seed)`, and the planned feature/rule history signatures.
Existing callers remain supported. The ID-only split helper assumes independent
IDs; the CLI uses complete episodes to keep repeated cases together.

The self-contained `simple_rule_action` matches the unchanged starter on every
training observation. Both constant baselines also passed the full training-state
smoke run. No value model has been fitted and no candidate performance is claimed.
Service-threshold alternatives are unit-tested but their development performance
must wait for Task 3's evaluator. Checkpoint work is retained on
`feat/checkpoint-one` without a merge or push.

Current-data review found no blocking issues. One existing generalization limit:
overlap counts describe usable episodes; the audit excludes quarantined records
from those checks. The supplied files have no quarantined records. Broader input
support should extend overlap auditing before relying on those counts for files
with quarantines.
